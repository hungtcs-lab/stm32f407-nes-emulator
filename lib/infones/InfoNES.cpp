...
/* RAM */
NES_FAST_RAM BYTE RAM[ RAM_SIZE ];

/* SRAM */
NES_FAST_RAM BYTE SRAM[ SRAM_SIZE ];
...
/* PPU RAM */
NES_FAST_RAM BYTE PPURAM[ PPURAM_SIZE ];
...
/* Character Buffer */
NES_FAST_RAM BYTE ChrBuf[ 256 * 2 * 8 * 8 ];
 ...
 */
  ...
  if ( FrameCnt == 0 &&
       PPU_ScanTable[ PPU_Scanline ] == SCAN_ON_SCREEN &&
       InfoNES_ShouldDrawLine( PPU_Scanline ) )   /* [port] 平台层可以跳过部分行（隔行渲染）*/
  {
    InfoNES_DrawLine();
  }
 ...
 */
  ...
  else
  {
    ...
    // Callback at PPU read/write
    if ( MapperPPU != Map0_PPU ) MapperPPU( PATTBL( pbyChrData ) );   /* [port] 跳过空回调，每行省 33 次函数指针调用 */
  ...
  if ( PPU_R1 & R1_SHOW_SP )
  {
    ...
    /* [port] 精灵缓冲改为找到第一个精灵时才清零，合成循环只扫有精灵的范围。
     * 大部分扫描线上没有精灵，原来每行都 memset 263 字节再逐像素扫 256 次 */
    int nSprMinX = NES_DISP_WIDTH, nSprMaxX = -1;
    ...
    for ( pSPRRAM = SPRRAM + ( 63 << 2 ); pSPRRAM >= SPRRAM; pSPRRAM -= 4 )
    {
     ...
     /*-------------------------------------------------------------------*/
      ...
      if ( ++nSprCnt == 1 )
        InfoNES_MemorySet( pSprBuf, 0, sizeof pSprBuf );   /* [port] 延迟清零 */
      ...
      nX = pSPRRAM[ SPR_X ];
      if ( nX < nSprMinX ) nSprMinX = nX;   /* [port] */
      if ( nX > nSprMaxX ) nSprMaxX = nX;
    ...
    // Rendering sprite
    pPoint = WorkLine;   /* [port] 原为 pPoint -= ( NES_DISP_WIDTH - PPU_Scr_H_Bit ); 关屏时会越界 */
    int nSprEnd = ( nSprMaxX + 8 < NES_DISP_WIDTH ) ? nSprMaxX + 8 : NES_DISP_WIDTH;
    for ( nX = nSprMinX; nX < nSprEnd; ++nX )   /* [port] 原为 0..NES_DISP_WIDTH */
    {
      nSprData = pSprBuf[ nX ];
      if ( nSprData  && ( nSprData & 0x80 || pPoint[ nX ] & NES_BG_FLAG ) )   /* [port] 原为 0x8000 */
      {
  ...
  /* [port] 一行画完，交给平台层输出 */
  InfoNES_LoadLine( PPU_Scanline, WorkLine );
}
